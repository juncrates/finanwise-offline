...
type CardData = {
  ...
  dueDay: number;
  closingDay: number;
  color: "purple" | "orange" | "yellow" | "green" | "blue" | "pink" | "red" | "graphite";
};
...
function cardColorForBank(bank: string): CardData["color"] {
  const value = bank.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();

  // 🟣 ROXO - Nubank, Digio, Will Bank
  if (
    value.includes("nubank") ||
    value.includes("ultravioleta") ||
    value.includes("digio") ||
    value.includes("will bank") ||
    value.includes("willbank")
  ) {
    return "purple";
  }

  // 🟠 LARANJA - Inter, Itaú, Pan
  if (
    value.includes("inter") ||
    value.includes("itau") ||
    value.includes("pan ") ||
    value.includes("panbank")
  ) {
    return "orange";
  }

  // 🟡 AMARELO - Meliuz, Banco do Brasil
  if (
    value.includes("meliuz") ||
    value.includes("banco do brasil") ||
    value.includes(" bb ") ||
    value.includes("bb ")
  ) {
    return "yellow";
  ...
  // 🟢 VERDE - PicPay, Neon, PagBank
  if (
    value.includes("picpay") ||
    value.includes("neon") ||
    value.includes("pagbank") ||
    value.includes("pagseguro")
  ) {
    return "green";
  }

  // 🔵 AZUL - Mercado Pago, Caixa, Magalu, BTG, RecargaPay
  if (
    value.includes("mercado pago") ||
    value.includes("mercadopago") ||
    value.includes("caixa") ||
    value.includes("magalu") ||
    value.includes("magazine luiza") ||
    value.includes("btg") ||
    value.includes("recargapay")
  ) {
    return "blue";
  }

  // 🩷 ROSA - Ame
  if (value.includes("ame") || value.includes("americanas")) {
    return "pink";
  }

  // 🔴 VERMELHO - Santander, Bradesco
  if (
    value.includes("santander") ||
    value.includes("bradesco") ||
...
function safeLoad(): Store {
  try {
    ...
    return { ...parsed, payments: parsed.payments ?? [], cards: (parsed.cards ?? []).map((card: CardData) => ({ ...card, dueDay: card.dueDay ?? 10, closingDay: card.closingDay ?? 3, color: cardColorForBank(`${card.bank} ${card.name}`) })) };
  } catch {
    return seedStore;
  }
}
...
export default function Home() {
  ...
  const installments = useMemo(() => {
    ...
    return Array.from({ length: finance.term }, (_, index) => {
      ...
      return { number, due, status: isPaid ? "paid" : due < today ? "late" : number === finance.paidCount + 1 ? "next" : "open" } as const;
    });
  }, [finance]);
  ...
  const debtors = useMemo(() => {
    ...
    store.expenses.forEach((expense) => {
      const remaining = expense.amount * Math.max(0, expense.installments - expense.installmentsPaid) / expense.installments;
  ...
  const updateFinance = (updater: (current: Finance) => Finance) => {
    setStore((current) => ({ ...current, finances: current.finances.map((item) => item.id === finance.id ? updater(item) : item) }));
  };

  const registerPayment = () => {
    if (!finance || finance.paidCount >= finance.term) return;
    const nextStore = { ...store, finances: store.finances.map((item) => item.id === finance.id ? { ...item, paidCount: item.paidCount + 1 } : item) };
  ...
  const registerFinanceInstallment = (number: number) => {
    ...
    const entry: PaymentEntry = { id: `payment-${Date.now()}`, kind: "finance", referenceId: finance.id, label: `Parcela ${number}ª · ${finance.lender}`, installmentNumber: number, scheduledAmount: finance.installment, paidAmount: paid, discount, paidAt: paymentDate };
    const nextStore = { ...store, finances: store.finances.map((item) => item.id === finance.id ? { ...item, paidCount: number } : item), payments: [entry, ...store.payments] };
    persistStore(nextStore);
    setStore(nextStore);
    setModal(null);
    setLaunchAmount("");
  ...
  const registerCardInstallment = (id: string) => {
    ...
    const entry: PaymentEntry = { id: `payment-${Date.now()}`, kind: "card", referenceId: currentExpense.id, label: currentExpense.description, installmentNumber: currentExpense.installmentsPaid + 1, scheduledAmount: scheduled, paidAmount: paid, discount, paidAt: paymentDate };
    const nextStore = {
      ...store,
      expenses: store.expenses.map((expense) => expense.id === id ? { ...expense, installmentsPaid: expense.installmentsPaid + 1 } : expense),
  ...
  const openCardEditor = (card: CardData) => {
    ...
    setCardForm({ name: card.name, bank: card.bank, lastFour: card.lastFour, limit: String(card.limit), dueDay: String(card.dueDay), closingDay: String(card.closingDay) });
    setModal("editCard");
  };
  ...
  const editCard = (event: React.FormEvent) => {
    ...
    const nextStore = { ...store, cards: store.cards.map((card) => card.id === editingCardId ? { ...card, name: cardForm.name.trim(), bank: cardForm.bank.trim(), lastFour: cardForm.lastFour.slice(-4), limit, dueDay, closingDay, color: cardColorForBank(`${cardForm.bank} ${cardForm.name}`) } : card) };
    persistStore(nextStore);
    setStore(nextStore);
    setModal(null);
  ...
  const deleteCard = (id: string) => {
    if (store.cards.length <= 1) { setToast("Mantenha pelo menos um cartão cadastrado."); return; }
    ...
    <div className="app-shell">
      <header className="topbar">
        <div className="brand-lockup">
          ...
          <div><span className="brand-name">finanwise</span><span className="brand-caption">offline</span></div>
        </div>
        <div className="topbar-actions">
          <span className="offline-pill"><span className="status-dot" /> local</span>
          <IconButton label="Notificações"><Bell size={19} /></IconButton>
          <IconButton label="Backup dos dados" onClick={() => setModal("backup")}><MoreHorizontal size={20} /></IconButton>
        </div>
      </header>
      ...
      <main className="main-content">
        <section className="welcome-row">
          <div>
            <p className="eyebrow">quinta-feira, 03 de setembro</p>
            <h1>Olá, Josenildo <span className="wave">✦</span></h1>
          ...
          <>
            <section className="hero-balance card-surface">
              <div className="balance-copy">
                <div className="balance-label"><span className="mini-icon violet"><WalletCards size={15} /></span> compromissos em aberto <span className="info-dot">i</span></div>
                <div className="balance-value">{money(remainingLoan + cardCommitment)}</div>
                <div className="balance-meta"><TrendingUp size={15} /> acompanhamento local atualizado</div>
              </div>
              <div className="balance-orbit"><CircleDollarSign size={94} strokeWidth={1.2} /></div>
              <div className="balance-footer"><span>financiamento</span><strong>{money(remainingLoan)}</strong><span className="divider-dot" /> <span>cartões este mês</span><strong>{money(cardCommitment)}</strong></div>
            </section>

            <div className="section-heading"><div><p className="eyebrow">acesso rápido</p><h2>O que você precisa?</h2></div><button className="text-button" onClick={() => setModal("backup")}>gerenciar dados <ChevronRight size={16} /></button></div>
            <div className="quick-actions">
              <button className="quick-card lilac" onClick={() => goTo("finances")}><span className="quick-icon"><Landmark size={20} /></span><span>Ver financiamento</span><ChevronRight size={16} /></button>
              <button className="quick-card mint" onClick={() => { setExpenseForm((form) => ({ ...form, cardId: selectedCardData?.id ?? "nubank" })); setModal("expense"); }}><span className="quick-icon"><ReceiptText size={20} /></span><span>Lançar despesa</span><Plus size={16} /></button>
              <button className="quick-card peach" onClick={() => goTo("reports")}><span className="quick-icon"><UsersRound size={20} /></span><span>Ver devedores</span><ChevronRight size={16} /></button>
            </div>

            <div className="section-heading compact"><div><p className="eyebrow">próximo compromisso</p><h2>Não deixe passar</h2></div><button className="circle-link" onClick={() => goTo("finances")}><ChevronRight size={18} /></button></div>
            <section className="next-payment card-surface">
              <div className="next-payment-main"><div className="payment-icon"><CarFront size={22} /></div><div><strong>Financiamento {finance?.lender}</strong><span>{nextInstallmentNumber}ª de {finance?.term} parcelas · vence {dateLabel(nextDue)}</span></div></div>
              <div className="next-payment-value"><strong>{money(finance?.installment ?? 0)}</strong><span className="status-chip amber">em aberto</span></div>
            </section>

            <div className="section-heading compact"><div><p className="eyebrow">seus números</p><h2>De relance</h2></div><button className="text-button" onClick={() => goTo("reports")}>relatórios <ChevronRight size={16} /></button></div>
            <div className="metric-grid">
              <article className="metric-card card-surface"><span className="metric-icon blue"><HandCoins size={19} /></span><div><span className="metric-label">parcelas pagas</span><strong>{finance?.paidCount ?? 0} <small>/ {finance?.term ?? 0}</small></strong></div><ProgressBar value={paidProgress} /></article>
              <article className="metric-card card-surface"><span className="metric-icon coral"><CreditCard size={19} /></span><div><span className="metric-label">fatura atual</span><strong>{money(cardCommitment)}</strong></div><span className="metric-foot">{store.expenses.length} despesas lançadas</span></article>
            </div>
          </>
        )}
        ...
        {activeTab === "finances" && (
          <section className="page-section">
            <div className="page-title-row"><div><p className="eyebrow">módulo 01</p><h1>Financiamentos</h1><p className="subtitle">Acompanhe o contrato e antecipe quando quiser.</p></div><button className="primary-button" onClick={() => setModal("anticipate")}><ArrowUpRight size={17} /> antecipar</button></div>
            <section className="finance-hero card-surface">
              <div className="finance-brand"><div className="bank-icon"><Landmark size={21} /></div><div><strong>{finance?.lender}</strong><span>Contrato {finance?.contract}</span></div><span className="status-chip green"><ShieldCheck size={13} /> em dia</span></div>
              <div className="finance-asset"><CarFront size={21} /><div><span>veículo financiado</span><strong>{finance?.vehicle}</strong><small>placa {finance?.plate}</small></div></div>
              <div className="finance-balance"><div><span className="metric-label">saldo projetado</span><strong>{money(remainingLoan)}</strong></div><div className="balance-side"><span>já pago</span><strong>{money((finance?.paidCount ?? 0) * (finance?.installment ?? 0))}</strong></div></div>
              <ProgressBar value={paidProgress} tone="green" />
              <div className="finance-progress-labels"><span>{finance?.paidCount} parcelas pagas</span><span>{finance?.term - (finance?.paidCount ?? 0)} restantes</span></div>
              <div className="finance-details"><div><span>parcela mensal</span><strong>{money(finance?.installment ?? 0)}</strong></div><div><span>juros a.m.</span><strong>{finance?.monthlyRate.toFixed(2).replace(".", ",")} %</strong></div><div><span>CET a.m.</span><strong>{finance?.cet.toFixed(2).replace(".", ",")} %</strong></div></div>
            </section>
            <div className="action-row"><button className="secondary-button" onClick={registerPayment}><Check size={16} /> registrar pagamento</button><button className="secondary-button" onClick={() => setModal("anticipate")}><ArrowUpRight size={16} /> antecipar parcelas</button></div>
            <div className="section-heading compact"><div><p className="eyebrow">linha do tempo</p><h2>Parcelas do contrato</h2></div><span className="muted-count">{finance?.term} no total</span></div>
            <div className="installment-list card-surface">{installments.slice(Math.max(0, (finance?.paidCount ?? 0) - 2), Math.min(12, (finance?.paidCount ?? 0) + 6)).map((item) => <div className={`installment-row ${item.status}`} key={item.number}><div className="installment-status">{item.status === "paid" ? <Check size={15} /> : item.status === "next" ? <span className="current-dot" /> : <CalendarDays size={15} />}</div><div className="installment-name"><strong>{item.number}ª parcela</strong><span>vencimento {dateLabel(item.due, "full")}</span></div><span className={`status-chip ${item.status === "paid" ? "green" : item.status === "next" ? "amber" : item.status === "late" ? "red" : "soft"}`}>{item.status === "paid" ? "paga" : item.status === "next" ? "próxima" : item.status === "late" ? "atrasada" : "em aberto"}</span><strong className="installment-amount">{money(finance?.installment ?? 0)}</strong>{item.status !== "paid" && <button className="row-action" onClick={() => { setLaunchingFinanceNumber(item.number); setLaunchAmount(String(finance?.installment ?? "")); setModal("launchFinance"); }}>lançar</button>}</div>)}</div>
            <div className="section-heading compact"><div><p className="eyebrow">histórico detalhado</p><h2>Pagamentos do financiamento</h2></div><span className="muted-count">{store.payments.filter((payment) => payment.kind === "finance").length} lançamentos</span></div>
            <div className="history-list card-surface">{store.payments.filter((payment) => payment.kind === "finance").slice(0, 8).map((payment) => <div className="history-row" key={payment.id}><div className="history-icon"><Check size={15} /></div><div className="history-copy"><strong>{payment.label}</strong><span>pago em {dateLabel(payment.paidAt, "full")}</span></div><div className="history-values"><strong>{money(payment.paidAmount)}</strong><span>{payment.discount > 0 ? `desconto ${money(payment.discount)}` : "sem desconto"}</span></div></div>)}{store.payments.filter((payment) => payment.kind === "finance").length === 0 && <div className="empty-state"><HandCoins size={25} /><strong>Nenhum pagamento lançado manualmente</strong><span>O histórico aparecerá quando você lançar uma parcela.</span></div>}</div>
            <p className="privacy-note"><LockKeyhole size={14} /> Seus dados ficam somente neste dispositivo.</p>
          </section>
        )}
        ...
        {activeTab === "cards" && (
          <section className="page-section">
            ...
            <div className="cards-scroller">{store.cards.length === 0 ? <div className="empty-state card-empty"><CreditCard size={26} /><strong>Nenhum cartão cadastrado</strong><span>Cadastre um cartão para começar a controlar suas faturas.</span></div> : store.cards.map((card) => { const cycle = statementKey(new Date().toISOString().slice(0, 10), card.closingDay); const outstanding = cardOutstandingAmount(card, store.expenses, store.payments); const detectedColor = cardColorForBank(`${card.bank} ${card.name}`); return <button key={card.id} className={`credit-card ${detectedColor} ${selectedCard === card.id ? "selected" : ""}`} onClick={() => setSelectedCard(card.id)}><div className="credit-card-top"><span>{card.bank}</span><CreditCard size={20} /></div><div className="credit-card-number">••••  ••••  ••••  {card.lastFour}</div><div className="credit-card-bottom"><div><small>limite disponível</small><strong>{money(card.limit - outstanding)}</strong></div><div className="card-holder">{card.name}</div></div></button>; })}</div>
        ...
        {activeTab === "reports" && (
          <section className="page-section">
            <div className="page-title-row"><div><p className="eyebrow">visão analítica</p><h1>Relatórios</h1><p className="subtitle">Saiba quanto está comprometido e com quem.</p></div><button className="secondary-button" onClick={exportBackup}><FileDown size={16} /> exportar</button></div>
            <div className="report-highlight card-surface"><div className="report-highlight-icon"><UsersRound size={23} /></div><div><span>total a receber</span><strong>{money(debtors.reduce((sum, [, value]) => sum + value, 0))}</strong><small>considerando parcelas futuras</small></div><ArrowUpRight size={20} /></div>
            <div className="report-metrics"><div className="report-mini card-surface"><span className="metric-icon blue"><CreditCard size={17} /></span><span>cartões ativos</span><strong>{store.cards.length}</strong></div><div className="report-mini card-surface"><span className="metric-icon coral"><ReceiptText size={17} /></span><span>despesas lançadas</span><strong>{store.expenses.length}</strong></div></div>
            <div className="section-heading compact"><div><p className="eyebrow">por pessoa</p><h2>Relatório de devedor</h2></div><span className="muted-count">{debtors.length} pessoas</span></div>
            <div className="debtor-list card-surface">{debtors.map(([person, value], index) => { const total = debtors.reduce((sum, [, amount]) => sum + amount, 0); return <div className="debtor-row" key={person}><div className={`person-avatar avatar-${index % 4}`}>{person.split(" ").map((part) => part[0]).join("").slice(0, 2)}</div><div className="debtor-copy"><strong>{person}</strong><span>{store.expenses.filter((expense) => expense.person === person).length} despesas vinculadas</span><ProgressBar value={total ? value / total * 100 : 0} tone={index === 0 ? "coral" : "blue"} /></div><div className="debtor-value"><strong>{money(value)}</strong><span>a receber</span></div></div>; })}</div>
            <div className="insight-banner"><Sparkles size={19} /><div><strong>Uma boa prática</strong><span>registre quem fez a compra para nunca perder a conta no fim do mês.</span></div></div>
          </section>
        )}
      </main>
